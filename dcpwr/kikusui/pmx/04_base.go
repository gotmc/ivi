...
func (ch *Channel) SetCurrentLimitBehavior(
	...
	case dcpwr.CurrentRegulate:
		...
		return ch.inst.Command(ctx, "CURR:PROT MAX")
	case dcpwr.CurrentTrip:
		ch.currentLimitBehavior = dcpwr.CurrentTrip
		limit, err := query.Float64(ctx, ch.inst, "CURR?")
		if err != nil {
			return err
		}
...
func (ch *Channel) SetOutputEnabled(v bool) error {
	...
	return ch.inst.Command(ctx, "OUTP 0")
}
...
func (ch *Channel) OVPEnabled() (bool, error) {
	...
	ovp, err := query.Float64(ctx, ch.inst, "VOLT:PROT?")
	if err != nil {
		return false, err
	}
	if ovp == max {
